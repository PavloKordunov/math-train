    ...
    return (
        <div className="flex items-stretch w-fit h-fit mb-8">
            ...
            <div className="px-10 border-[2px] border-[#CDC8C8] border-l-0 flex items-center justify-between min-w-[900px] gap-10">
                <p className="font-bold text-[18px] uppercase">{test?.testName}, Здано: {formatDateToUkrainian(test?.createdAt)}, Оцінка: {test.score}/{test.maxScore}</p>
                <Link href={`/perfomence/${test.id}`} className="bg-[#CA193A] px-4 py-2 text-white rounded-md font-semibold uppercase">Переглянути тест</Link>
            </div>
        </div>
    );
}
