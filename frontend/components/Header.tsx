            ...
            <div className="flex gap-12">
                <Link
                    ...
                    className={`text-[20px] cursor-pointer relative transition duration-200 ${
                        activeTab === "home" ? "text-[#FA8E66] font-bold" : "text-black font-medium"
                    }`}
                >
                    Домашня
                    {activeTab === "home" && (
                ...
                <Link 
                    href={`${user?.status === 'Student' ? '/perfomence' : '/teacher-performance'}`}
                    className={`text-[20px] cursor-pointer relative transition duration-200 ${
                        activeTab === "map" ? "text-[#FA8E66] font-bold" : "text-black font-medium"
                    }`}
                >
            ...
            <div className="relative">
                ...
                {isDropdownOpen && (
                    <div className="absolute right-0 mt-3 bg-white shadow-lg rounded-xl w-52 py-3 px-2 z-50 border border-gray-200">
                        <Link
                             href={`${user?.status === 'Student' ? `/student/${user?.id}` : 'teacher'}`}
