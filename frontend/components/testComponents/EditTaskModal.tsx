...
const EditTaskModal = memo(
    ({
        ...
        const validatePairsRealtime = (pairs: any[]): string | '' => {
            if (pairs.length < 1) {
        ...
        const validateBeforeSave = (): boolean => {
            ...
            if (editedTask.type === 'multiple') {
                if (!editedTask.answers || editedTask.answers.length < 1) {
                    newErrors.sizeAnswer =
                        'Потрібно додати хоча б одну відповідь'
                    valid = false
                } else {
        ...
        const handleRemoveImage = (type: string) => {
            setEditedTask((prev: any) => {
                ...
                if (
                    prev.answers.some((ans: any) => `answer${ans.id}` === type)
                ) {
                    return {
                        ...prev,
        ...
        return (
            ...
            >
                <div
                    ...
                    className="bg-white rounded-[31px] p-6 w-full max-w-200 max-h-[90vh] overflow-y-auto"
                >
