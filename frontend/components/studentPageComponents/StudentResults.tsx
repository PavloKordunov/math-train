...
const StudentResults = ({studentPromise, studentResultsPromise, assignedTestsPromise}: any) => {
            ...
            {user?.status === "Teacher" && (
                <div>
                    <div className="flex gap-8 my-10">
                        ...
                        className={`font-bold text-[28px] cursor-pointer relative transition duration-200 ${active ==="passTests" ? "text-[#CA193A]" : "text-black"} `}
                        >
                            Пройдені тести
                            {active === 'passTests' && (
                        ...
                        className={`font-bold text-[28px] cursor-pointer relative transition duration-200 ${active ==="activeTests" ? "text-[#CA193A]" : "text-black"} `}
                        >
                            Активні тести
                            {active === 'activeTests' && (
                                <span className="absolute left-0 -bottom-1 w-full h-[2px] bg-[#CA193A]"></span>
                    ...
                    {active === 'activeTests' && (
                        <div>
                            {assignedTests.length > 0 ? assignedTests.map((test: any) => (
